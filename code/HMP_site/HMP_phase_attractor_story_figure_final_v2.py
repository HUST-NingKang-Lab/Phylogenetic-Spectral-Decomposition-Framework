...
OUT_DIR = Path(r"/home/zhangyuli/傅里叶非时序/figures/HMP_site/phase")
...
def plot_polar_modes(fig, gs_cell, ptab: pd.DataFrame, selected: List[int], anchor_labels: pd.DataFrame) -> None:
    ...
    for idx, k in enumerate(selected):
        ...
        for site in PREFERRED_BODY_SITES:
            ...
            if len(vals) > 150:
                vals = rng.choice(vals, size=150, replace=False)
            radii = 0.92 + 0.05 * rng.normal(size=len(vals))
            ax.scatter(vals, radii, s=4, color=SITE_COLORS.get(site, 'grey'), alpha=0.13, edgecolor='none')
            mu = circ_mean(ss['phase'].values)
            r = max(circ_r(ss['phase'].values), 0.06)
            ax.plot([mu, mu], [0, r], color=SITE_COLORS.get(site, 'grey'), lw=1.45)
            ...
            if label:
                rt = min(1.13, r + 0.11)
                ax.text(mu, rt, label, color=SITE_COLORS.get(site, 'grey'), fontsize=5.2,
