...
def plot_low_high_locking(ax, site_stats: pd.DataFrame) -> pd.DataFrame:
    order = ['low', 'mid', 'high']
    labels = ['Low\n(k≤12)', 'Mid\n(13–32)', 'High\n(>32)']
    data = [site_stats.loc[site_stats['band'].eq(b), 'PPC'].dropna().values for b in order]
    bp = ax.boxplot(data, patch_artist=True, showfliers=False, widths=0.6)
    colors = ['#E8C98C', '#B8D3D0', '#DDD8CD']
    for patch, c in zip(bp['boxes'], colors):
        patch.set_facecolor(c); patch.set_edgecolor(COL['dark']); patch.set_alpha(0.9)
    ...
    for i, vals in enumerate(data, start=1):
        ...
        ax.scatter(np.full(len(vals), i) + jitter, vals, s=12, color=COL['oral'], alpha=0.45, edgecolor='white', lw=0.2, zorder=3)
    ...
    style_axis(ax)
    pairs = [(0,1), (0,2), (1,2)]
    pair_names = [('low','mid'), ('low','high'), ('mid','high')]
    pair_ps = []
    top = max([np.nanmax(v) if len(v) else 0 for v in data] + [0])
    base = top + 0.05
    step = 0.08
    for idx, ((i,j), (a,b)) in enumerate(zip(pairs, pair_names)):
        p = np.nan
        if len(data[i]) and len(data[j]):
            _, p = mannwhitneyu(data[i], data[j], alternative='two-sided')
        pair_ps.append({'comparison': f'{a}_vs_{b}', 'p_value': p})
        _add_sig_bracket(ax, i+1, j+1, base + idx*step, 0.025, _format_p(p))
    ...
    ax.set_ylim(bottom=min(0, site_stats['PPC'].min()-0.05), top=base + 3*step + 0.08)
    stats = pd.DataFrame(pair_ps)
    stats['overall_kruskal_H'] = H
    stats['overall_permutation_p'] = p_perm
    return stats
    ...
    for idx, k in enumerate(selected):
        ...
        for site in PREFERRED_BODY_SITES:
            ...
            if not anchor_labels.empty and (int(k), site) in lbl.index:
                label = str(lbl.loc[(int(k), site), 'top_genus'])
            if label:
                rt = min(1.13, r + 0.11)
...
def plot_longitudinal(ax, dist_df: pd.DataFrame) -> None:
    ...
    top = max([np.nanmax(v) if len(v) else 0 for v in data] + [0])
    base = top + 0.08
    comps = [((0,1),'same-vs-diff subject'), ((0,2),'same-vs-diff site')]
    for idx, ((i,j), name) in enumerate(comps):
        p = np.nan
        if len(data[i]) and len(data[j]):
            _, p = mannwhitneyu(data[i], data[j], alternative='two-sided')
        _add_sig_bracket(ax, i+1, j+1, base + idx*0.12, 0.03, _format_p(p))
    ax.set_ylim(bottom=min(0, np.nanmin(np.concatenate([d for d in data if len(d)]))-0.05), top=base+0.32)
...
def plot_backprojected_waves(ax, genus_anchor_df: pd.DataFrame, tax_order: pd.DataFrame) -> None:
    ...
    for site, y in y_sites.items():
        ...
        if sub.empty:
            continue
        size = 18 + 90 * (sub['anchor_score'] - sub['anchor_score'].min()) / (sub['anchor_score'].max() - sub['anchor_score'].min() + 1e-12)
        ax.scatter(sub['position'], np.full(len(sub), y), s=size, color=SITE_COLORS.get(site, 'grey'), alpha=0.88, edgecolor='white', lw=0.35, zorder=3)
        top_lab = sub.head(4).sort_values('position')
        for _, row in top_lab.iterrows():
            ax.text(row['position'], y + 0.14, str(row['genus']), color=SITE_COLORS.get(site, 'grey'), fontsize=5.8,
