...
def plot_polar_modes(fig, gs_cell, ptab: pd.DataFrame, selected: List[int], anchor_labels: pd.DataFrame) -> None:
    ...
    for idx, k in enumerate(selected):
        ...
        for sidx, site in enumerate(PREFERRED_BODY_SITES):
            ...
            sc = ax.scatter(vals, radii, s=9, color=color, alpha=0.22, edgecolor='white', linewidth=0.18)
            if idx == 0:
            ...
            if label:
                rt = min(1.12, r + 0.13)
                ha = 'left' if np.cos(mu) >= 0 else 'right'
                ax.text(mu, rt, label, color=color, fontsize=5.0, ha=ha, va='center', clip_on=False)
