...
def plot_score_permutation(ax, score_df: pd.DataFrame) -> None:
    """
    Plot phase-attractor scores across FFT modes.

    Modified version:
    - All observed points use the same color.
    - No color distinction between low-frequency and high-frequency modes.
    - Label-permutation background and null median are kept.
    """

    # Color settings
    observed_color = "#D75A49"   # same color for all observed points
    null_color = COL["null"]     # usually "#BDB7AA"

    # Label-permutation background: 5–95% interval
    ax.fill_between(
        score_df["mode"],
        score_df["null_q05"],
        score_df["null_q95"],
        color=null_color,
        alpha=0.35,
        label="label perm. 5–95%"
    )

    # Null median line
    ax.plot(
        score_df["mode"],
        score_df["null_median"],
        color=null_color,
        lw=1.0,
        label="null median"
    )

    # Observed phase-attractor scores
    ax.scatter(
        score_df["mode"],
        score_df["phase_attractor_score"],
...
def select_anchor_and_de_taxa(genus_anchor_df: pd.DataFrame,
    ...
    anchor_rows, de_rows = [], []
    # clr is samples x genera. Duplicate genus names can occur after genus-level
    # aggregation / phylogeny matching. Collapse them first; otherwise feature_df.loc[:, g]
    # may return a DataFrame and the rank-sum effect becomes an array, causing pandas
    # replace/dropna errors downstream.
    feature_df = clr.copy()
    feature_df.columns = feature_df.columns.astype(str)
    if feature_df.columns.duplicated().any():
        feature_df = feature_df.T.groupby(level=0).mean().T
    ...
    for site in PREFERRED_BODY_SITES:
        ...
        for g in feature_df.columns.astype(str):
            vals_pos = np.asarray(feature_df.loc[y, g]).astype(float).ravel()
            vals_neg = np.asarray(feature_df.loc[~y, g]).astype(float).ravel()
...
def compute_anchor_vs_de_roc(anchor_taxa_df: pd.DataFrame,
    ...
    # clr is samples x genera; collapse duplicate genus columns for robust model input.
    X_all = clr.copy()
    X_all.columns = X_all.columns.astype(str)
    if X_all.columns.duplicated().any():
        X_all = X_all.T.groupby(level=0).mean().T
...
def create_roc_comparison_supplementary(roc_results: Dict[str, Dict[str, object]], auc_df: pd.DataFrame) -> None:
    ...
    for idx, site in enumerate(site_list):
        ...
        ax.plot([0,1], [0,1], ls='--', lw=0.9, color=COL.get('muted', '#AFA99B'))
