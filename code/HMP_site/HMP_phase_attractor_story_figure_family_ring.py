...
OUT_DIR = Path(r"/home/zhangyuli/傅里叶非时序/figures/HMP_site/phase_new")
...
def plot_low_high_locking(ax, site_stats: pd.DataFrame) -> pd.DataFrame:
    ...
    high_vals = site_stats.loc[~site_stats['band'].eq('low'), 'PPC'].dropna().values
    data = [low_vals, high_vals]
    labels = ['Low-order\n(k≤12)', 'High-order\n(k>12)']
    bp = ax.boxplot(data, patch_artist=True, showfliers=False, widths=0.58)
    colors = ['#E8C98C', '#C5D7D9']
    for patch, c in zip(bp['boxes'], colors):
        patch.set_facecolor(c); patch.set_edgecolor(COL['dark']); patch.set_alpha(0.92)
    ...
    for i, vals in enumerate(data, start=1):
        ...
        ax.scatter(np.full(len(vals), i) + jitter, vals, s=12, color=COL['oral'], alpha=0.45,
    ...
    stats = pd.DataFrame([
        {'comparison': 'low_vs_high', 'p_value': p_mw, 'overall_kruskal_H': H, 'overall_permutation_p': p_perm}
    ])
    return stats
...
def _build_family_ring_layout(tax_order: pd.DataFrame) -> pd.DataFrame:
    work = tax_order.copy().reset_index(drop=True)
    work['family'] = work.get('family', pd.Series(['Unknown'] * len(work))).fillna('Unknown').astype(str)
    work['genus'] = work['genus'].astype(str)
    rows = []
    for fam, sub in work.groupby('family', sort=False):
...
def plot_polar_modes(fig, gs_cell, ptab: pd.DataFrame, selected: List[int], anchor_labels: pd.DataFrame, tax_order: pd.DataFrame) -> None:
    inner = gs_cell.subgridspec(1, len(selected), wspace=0.42)
    lbl = anchor_labels.set_index(['mode', 'body_site']) if not anchor_labels.empty else pd.DataFrame()
    ring_df = _build_family_ring_layout(tax_order)
    n_genus = len(tax_order)
    genus_pos = {str(g): i for i, g in enumerate(tax_order['genus'].astype(str).tolist())}
    highlight_fams = set(anchor_labels.loc[anchor_labels['mode'].isin(selected), 'top_family'].dropna().astype(str).tolist())
    ...
    for idx, k in enumerate(selected):
        ...
        ax.set_yticklabels([])
        ax.set_xticklabels([])
        ax.grid(color=COL['light'], lw=0.55)
        ax.spines['polar'].set_visible(False)
        # family ring
        for j, row in ring_df.iterrows():
            theta = 2 * np.pi * ((row['center'] + 0.5) / n_genus)
            width = 2 * np.pi * (max(row['width_n'], 1) / n_genus)
            is_hi = row['family'] in highlight_fams
            face = '#EFEAE0' if j % 2 == 0 else '#E3DDD2'
            if is_hi:
                face = '#D9CBB2'
            ax.bar(theta, 0.12, width=width, bottom=1.02, color=face,
                   edgecolor='white', lw=0.5, align='center', zorder=1)
        # label only highlighted families to avoid clutter
        lab_df = ring_df[ring_df['family'].isin(highlight_fams)].copy().sort_values('center')
        for _, row in lab_df.iterrows():
            ...
            if 90 < deg < 270:
                rot += 180
                ha = 'right'
            fam_txt = row['family'] if len(row['family']) <= 16 else row['family'][:15] + '…'
            ax.text(theta, 1.20, fam_txt, rotation=rot, rotation_mode='anchor',
                    ha=ha, va='center', fontsize=4.7, color=COL['dark'], clip_on=False)

        # site arrows point to representative genus position within the family ring
        for site in PREFERRED_BODY_SITES:
            ss = ptab[(ptab['mode'].eq(k)) & (ptab['body_site'].eq(site))]
            if ss.empty:
                continue
            mu = circ_mean(ss['phase'].values)
            r = max(circ_r(ss['phase'].values), 0.10)
            angle = mu
            gen = fam = ''
            if not anchor_labels.empty and (int(k), site) in lbl.index:
                gen = str(lbl.loc[(int(k), site), 'top_genus'])
                fam = str(lbl.loc[(int(k), site), 'top_family'])
                if gen in genus_pos:
                    angle = 2 * np.pi * ((genus_pos[gen] + 0.5) / n_genus)
                elif fam in set(ring_df['family']):
                    center = float(ring_df.loc[ring_df['family'].eq(fam), 'center'].iloc[0])
                    angle = 2 * np.pi * ((center + 0.5) / n_genus)
            ax.plot([angle, angle], [0.05, 0.93], color=SITE_COLORS.get(site, 'grey'), lw=1.55, zorder=3)
            ax.scatter([angle], [0.93], s=20, color=SITE_COLORS.get(site, 'grey'), edgecolor='white', lw=0.35, zorder=4)
            # small genus label near endpoint; family is already shown on outer ring
            if gen:
                t = gen if len(gen) <= 13 else gen[:12] + '…'
                ...
                if 90 < deg < 270:
                    rot += 180
                    ha = 'right'
                ax.text(angle, 0.80, t, color=SITE_COLORS.get(site, 'grey'), fontsize=4.8,
                        rotation=rot, rotation_mode='anchor', ha=ha, va='center')
        ax.set_title(f'k={k}', pad=7)
...
def plot_backprojected_waves(ax, genus_anchor_df: pd.DataFrame, tax_order: pd.DataFrame) -> None:
    ...
    work = genus_anchor_df.copy()
    work['genus'] = work['genus'].astype(str)
    work['position'] = work['genus'].map(pos_map)
    work = work.dropna(subset=['position'])
    # choose a compact union of top anchor genera across body sites, ordered by phylogenetic position
    selected = []
    for site in PREFERRED_BODY_SITES:
        sub = work[work['body_site'].eq(site)].sort_values('anchor_score', ascending=False).head(6)
        selected.append(sub[['genus', 'position', 'family']])
    sel = pd.concat(selected, ignore_index=True).drop_duplicates(subset=['genus']).sort_values('position')
    if sel.empty:
        ax.text(0.5, 0.5, 'No mappable anchor genera', ha='center', va='center', transform=ax.transAxes)
        ax.set_axis_off(); return
    genera = sel['genus'].tolist()
    families = sel['family'].astype(str).tolist() if 'family' in sel.columns else ['Unknown'] * len(sel)
    mat = (work[work['genus'].isin(genera)]
           .pivot_table(index='body_site', columns='genus', values='anchor_score', aggfunc='max')
           .reindex(index=PREFERRED_BODY_SITES, columns=genera)
           .fillna(0.0))
    vmax = np.nanpercentile(mat.values, 95) if np.isfinite(mat.values).any() else 1.0
    im = ax.imshow(mat.values, aspect='auto', cmap='RdBu_r', vmin=-vmax, vmax=vmax)
    ax.set_yticks(np.arange(len(PREFERRED_BODY_SITES))); ax.set_yticklabels(PREFERRED_BODY_SITES)
    xlabels = []
    for g, fam in zip(genera, families):
        short = g if len(g) <= 13 else g[:12] + '…'
        xlabels.append(short)
    ax.set_xticks(np.arange(len(genera))); ax.set_xticklabels(xlabels, rotation=50, ha='right')
    ax.set_xlabel('Top anchor genera ordered by phylogenetic position')
    ax.set_title('Body-site anchor genera reveal positional shifts along the ordered axis')
    for sp in ax.spines.values(): sp.set_visible(False)
    ax.tick_params(length=0)
    # annotate family names above selected strongest columns only
    col_strength = mat.abs().max(axis=0).sort_values(ascending=False)
    top_cols = set(col_strength.head(min(10, len(col_strength))).index)
    for j, (g, fam) in enumerate(zip(genera, families)):
        if g in top_cols:
            ax.text(j, -0.62, fam if len(fam) <= 14 else fam[:13] + '…', rotation=35,
                    ha='left', va='bottom', fontsize=5.4, color=COL['dark'], clip_on=False)
    cb = plt.colorbar(im, ax=ax, fraction=0.025, pad=0.012)
    cb.set_label('Anchor score')
...
def create_figure(res: Dict[str, object]) -> None:
    ...
    plot_polar_modes(fig, gs[1, :], res['phase_table'], res['selected_polar_modes'], res['mode_anchor_labels'], res['tax_order'])
...
def run_analysis() -> Dict[str, object]:
    ...
    selected_polar = [k for k in [1, 2, 3, 4, 5] if k in modes]
