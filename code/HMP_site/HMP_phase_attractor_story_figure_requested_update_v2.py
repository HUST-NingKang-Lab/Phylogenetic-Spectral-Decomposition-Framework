...
def select_anchor_and_de_taxa(genus_anchor_df: pd.DataFrame,
    ...
    anchor_rows, de_rows = [], []
    feature_df = clr.T.copy()
    feature_df['body_site'] = meta.set_index('sample').loc[feature_df.index, 'body_site']
    for site in PREFERRED_BODY_SITES:
        sub = genus_anchor_df[genus_anchor_df['body_site'].eq(site)].copy()
        sub = sub.sort_values('anchor_score', ascending=False).head(top_n)
        anchor_g = sub['genus'].astype(str).tolist()
        for rank, g in enumerate(anchor_g, start=1):
            anchor_rows.append({'body_site': site, 'genus': g, 'rank': rank, 'set': 'anchor'})
        # differential taxa: one-vs-rest rank-sum on CLR, select same number, prefer positively enriched taxa
        y = (feature_df['body_site'].values == site)
        stats = []
        for g in clr.index.astype(str):
            ...
            stats.append((g, p, eff))
        st = pd.DataFrame(stats, columns=['genus','p_value','effect'])
        if st.empty:
            continue
        st = st.replace([np.inf, -np.inf], np.nan).dropna()
        # BH adjustment
        ...
        if len(st_pos) < top_n:
            st_sel = pd.concat([st_pos, st[~st['genus'].isin(st_pos['genus'])].sort_values(['q_value','p_value','effect'], ascending=[True,True,False])]).head(top_n)
        else:
            st_sel = st_pos.head(top_n)
...
def compute_anchor_vs_de_roc(anchor_taxa_df: pd.DataFrame,
    ...
    X_all = clr.T.copy()
    y_site = meta.set_index('sample').loc[X_all.index, 'body_site']
    results = {}
    auc_rows = []
