...
OUT_DIR = Path(r"/home/zhangyuli/傅里叶非时序/figures/HMP_site/phase_taxon_backprojection")
...
def plot_low_high_locking(ax, site_stats: pd.DataFrame) -> None:
    # average PPC across sites by mode
    summ = site_stats.groupby("mode")["PPC"].mean().reset_index()
    ax.plot(summ["mode"], summ["PPC"], color=COL["dark"], lw=1.3)
    ax.scatter(summ["mode"], summ["PPC"], s=10, color=COL["oral"], edgecolor="white", lw=0.2, zorder=3)
    ax.axvspan(1, LOW_MAX_K, color="#EEDFB7", alpha=0.35, lw=0, label="low")
    ax.axvspan(LOW_MAX_K + 0.5, MID_MAX_K, color="#D8E5E3", alpha=0.25, lw=0, label="mid")
    ax.axvspan(MID_MAX_K + 0.5, MAX_MODE_INDEX, color="#EEECE5", alpha=0.5, lw=0, label="high")
    ax.axhline(0, color=COL["grey"], ls="--", lw=0.8)
    ax.set_xlabel("FFT mode index")
    ax.set_ylabel("Mean within-site PPC")
    ax.set_title("Low-order modes show stronger phase locking")
    style_axis(ax)
...
def plot_polar_modes(fig, gs_cell, ptab: pd.DataFrame, selected: List[int]) -> None:
    # selected length exactly 3 preferred
    ...
    for idx, k in enumerate(selected):
        ...
        for site in PREFERRED_BODY_SITES:
            ...
            if ss.empty:
                continue
            rng = np.random.default_rng(RANDOM_STATE + int(k) + idx)
            # very light sample phase cloud at radius ~1
            vals = ss["phase"].values
            if len(vals) > 180:
                vals = rng.choice(vals, size=180, replace=False)
            radii = 0.93 + 0.055 * rng.normal(size=len(vals))
            ax.scatter(vals, radii, s=4, color=SITE_COLORS.get(site, "grey"), alpha=0.14, edgecolor="none")
            mu = circ_mean(ss["phase"].values)
            r = circ_r(ss["phase"].values)
            ax.plot([mu, mu], [0, r], color=SITE_COLORS.get(site, "grey"), lw=1.6)
            ax.scatter([mu], [r], s=18, color=SITE_COLORS.get(site, "grey"), edgecolor="white", lw=0.35, zorder=5)
...
def plot_longitudinal(ax, dist_df: pd.DataFrame) -> None:
    if dist_df.empty:
        ...
        ax.set_axis_off(); return
    order = ["same subject-site", "different subject same site", "same subject different site", "random different site"]
    ...
    colors = ["#7FA3A8", "#B8C7D2", "#D77A61", "#CFC9BC"]
    ...
    ax.set_xticklabels(["same\nsubject-site", "different\nsubject", "same subject\ndifferent site", "random\ndifferent site"], rotation=0)
    ax.set_ylabel("Mean phase distance")
    ax.set_title("Body-site phases behave as longitudinal attractors")
    style_axis(ax)
...
def plot_backprojected_waves(ax, site_waves: Dict[str, np.ndarray], selected_modes: List[int], tax_order: pd.DataFrame) -> None:
    x = np.arange(len(tax_order))
    offset_step = 2.6
    for i, site in enumerate(PREFERRED_BODY_SITES):
        if site not in site_waves:
            continue
        y = site_waves[site] + i * offset_step
        ax.plot(x, y, lw=1.1, color=SITE_COLORS.get(site, "grey"), label=site)
    ax.set_xlabel("Ordered genera")
    ax.set_ylabel("Locked waveform, offset")
    ax.set_title(f"Site-specific phase positions projected to genus axis (modes {'+'.join(map(str, selected_modes[:8]))}{'...' if len(selected_modes)>8 else ''})")
...
def create_figure(res: Dict[str, object]) -> None:
    ...
    gs = GridSpec(4, 4, figure=fig, height_ratios=[0.95, 1.15, 1.15, 1.05], hspace=0.95, wspace=0.65)
    ...
    panel_label(ax, "b")
    plot_low_high_locking(ax, res["site_stats"])

    ax = fig.add_subplot(gs[0, 2])
    panel_label(ax, "c")
    plot_rayleigh_heatmap(ax, res["site_stats"])

    ax = fig.add_subplot(gs[0, 3])
    panel_label(ax, "d")
    plot_score_permutation(ax, res["score_df"])

    dummy = fig.add_subplot(gs[1, :3]); dummy.set_axis_off(); panel_label(dummy, "e")
    plot_polar_modes(fig, gs[1, :3], res["phase_table"], res["selected_polar_modes"])
    ...
    ax = fig.add_subplot(gs[2, 2:])
    panel_label(ax, "h")
    plot_pairwise_clade_shift(ax, res["clade_shift_df"])
    ...
    ax = fig.add_subplot(gs[3, 2:])
    panel_label(ax, "j")
    plot_backprojected_waves(ax, res["site_waves"], res["selected_backprojection_modes"], res["tax_order"])

    fig.suptitle("Body-site spectral phase attractors trace back to taxonomic anchors", y=0.995, fontsize=12, color=COL["dark"])
    fig.savefig(OUT_DIR / "figure_HMP_phase_taxon_backprojection.png", bbox_inches="tight")
    fig.savefig(OUT_DIR / "figure_HMP_phase_taxon_backprojection.pdf", bbox_inches="tight")
    plt.close(fig)
...
def run_analysis() -> Dict[str, object]:
    ...
    selected_polar = cand.head(TOP_ATTRACTOR_MODES)["mode"].astype(int).tolist()
    while len(selected_polar) < TOP_ATTRACTOR_MODES:
        for k in [1, 2, 3, 4, 5, 6]:
            if k in modes and k not in selected_polar:
                selected_polar.append(k)
            if len(selected_polar) == TOP_ATTRACTOR_MODES:
                break
